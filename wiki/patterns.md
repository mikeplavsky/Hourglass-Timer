...
**Where**: `tick_countdown` ([[src/timer.rs|timer.rs]]); `lerp_f32`/`interpolate_bulb_style`/`interpolate_neck_style`/`get_morphed_shape_config`/`within_click_radius`/`exceeds_drag_threshold` ([[src/hourglass.rs|hourglass.rs]]); `pick_distinct_color`/`color_dist_sq`/`rainbow_hue`/`hsl_to_rgb` ([[src/ui/color_panel.rs|color_panel.rs]]); `pick_distinct_shape`/`shape_button_scale`/`shape_row_contains_cursor` ([[src/ui/shape_panel.rs|shape_panel.rs]]); `pause_overlay_should_show` ([[src/ui/pause_overlay.rs|pause_overlay.rs]]); `add_time`/`format_time`/`reset` ([[src/resources.rs|resources.rs]]).
**Why**: Testability. Together with headless-`App` system tests (below), this discipline supports 90 regular Rust tests, 106 extension-feature Rust tests, and the JavaScript lifecycle suite. The flip side — logic left *inside* a camera/window-gated system stays untested. See [[references/test-coverage]].
