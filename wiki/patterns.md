...
**Why**: Testability. Together with headless-`App` system tests (below), this discipline supports 86 regular Rust tests, 102 extension-feature Rust tests, and the JavaScript lifecycle suite. The flip side — logic left *inside* a camera/window-gated system stays untested. See [[references/test-coverage]].
